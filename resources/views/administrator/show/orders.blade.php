...
<ul>
    ...
    <article class="command mb-16">
        ...
        <h2 class="uppercase">Commande traitée ! Numéro de suivi : {{ $order->track_number }}</h2>
        @else
        <form class="flex flex-nowrap" method="POST" action="{{ route('administrator.update.order') }}">
            @csrf
            <input name="order_id" type="hidden" value="{{ $order->id }}" />
            <input type="text" name="track_number" class="h-min w-3/4 mr-4 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Numéro de suivi" required>
