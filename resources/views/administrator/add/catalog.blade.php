...
<div>
    ...
    <form @if(isset($catalog)) action="{{ route('update.catalog', $catalog->id) }}" @else action="{{ route('store.catalog') }}" @endif enctype="multipart/form-data" method="POST">
        ...
        <input type="text" name="gender" id="gender" minlength="2" maxlength="60" @if(isset($catalog)) value="{{ $catalog->gender }}" @endif class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Nom du catalogue (exemple : homme)" required>
