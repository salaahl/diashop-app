...
<div>
    ...
    <form @if(isset($category)) action="{{ route('update.category', $category->id) }}" @else action="{{ route('store.category') }}" @endif enctype="multipart/form-data" method="POST">
        ...
        <div class="mb-10">
            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 uppercase">Catégorie</label>
            <input type="text" name="name" id="name" minlength="2" maxlength="60" @if(isset($category)) value="{{ $category->name }}" @endif class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Nom de la catégorie (exemple : vestes et manteaux)" required>
        </div>
        <div>
