...
<div class="flex flex-wrap flex-col lg:flex-row">
    <section id="product-images-container" class="h-[70vh] lg:h-auto w-full lg:w-2/4 lg:min-h-screen">
    ...
    <section id="product-details-container" class="w-full lg:w-2/4 min-h-[50vh] lg:min-h-[90vh]">
      <div id="product-detail" class="h-[80vh] sticky top-0">
        <div>
        ...
        <div>
          ...
          <div id="buttons">
            ...
            <button id="add-favorite"
              class="button-stylised-1 button-stylised-1-custom w-fit flex justify-center items-center whitespace-nowrap"
              role="button">
          ...
          <div id="delivery-and-return-details">
            ...
            <h4 class="text-sm text-gray-500">Retour possible sous quatorze jours à compter de la date de livraison.
            </h4>
          </div>
        </div>
      </div>
    ...
    @foreach($product->take(3) as $product)
    @if($product->id !== basename(url()->current()))
    <x-product
      link="/{{ $product->catalog->gender == 'Femme'?'woman':'men' }}/catalog/{{ $product->category->name }}/{{ $product->name }}/{{ $product->id }}"
      image="/images/{{ $product->img_thumbnail[0] }}" hover="/images/{{ $product->img_thumbnail[1] }}"
